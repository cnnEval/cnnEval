...
print(f'Running {EXPERIMENT}, split is {METHOD}, divisor is {DIVISOR}, run {RUN}.')

DATATYPE = eval(f'C.Figure4.data_to_{EXPERIMENT}')

OUTPUT_DIR = f'results/data/constrained_ratio/{METHOD}_{DIVISOR}/{EXPERIMENT}/'
OUTPUT_DIR_MODEL = f'results/model/constrained_ratio/{METHOD}_{DIVISOR}/{EXPERIMENT}/'
...
## Cleveland & McGill: 10 heights
Hcm = [10., 12., 15., 18., 22., 26., 32., 38., 46., 56.]
...
for p in Pcm:
    ...
    if r not in ratio_pair_cm:
        ...
        ratio_pair_cm[r].append(p)

all_ratios_cm = sorted(ratio_pair_cm.keys())

trainNum = int(round(len(all_ratios) * 0.6))
testNum = 23  #int(round(len(all_ratios) * 0.2))
valNum = len(all_ratios) - trainNum - testNum

# test ratios:
test_ratios = []
test_ratios.append(all_ratios_cm[0])                        # 0.18
# test_ratios.append(random.choice(all_ratios_cm[1:3]))     # 0.21, 0.22
test_ratios.extend(all_ratios_cm[3:5])                      # 0.26, 0.27
# test_ratios.append(random.choice(all_ratios_cm[5:8]))     # 0.31, 0.32, 0.33
test_ratios.extend(all_ratios_cm[8:10])                     # 0.38, 0.39
test_ratios.extend(all_ratios_cm[10:14])                    # 0.45, 0.46, 0.47, 0.48
test_ratios.extend(all_ratios_cm[14:18])                    # 0.55, 0.56, 0.57, 0.58
test_ratios.extend(all_ratios_cm[18:22])                    # 0.67, 0.68, 0.69, 0.7
test_ratios.extend(all_ratios_cm[22:28])                    # 0.8, 0.81, 0.82, 0.83, 0.84, 0.85

# val ratios:
val_ratios = []
for r in all_ratios:
    if r not in test_ratios:
        val_ratios.append(r)
    if len(val_ratios) == valNum:
        break

# train_ratios, all by default
train_ratios = []
for r in all_ratios:
    if r not in test_ratios and r not in val_ratios:
        train_ratios.append(r)
    if len(train_ratios) == trainNum:
        break
...
while test_counter < test_target:
    ...
    data = random.choice(ratio_pair_cm[ratio])
    random.shuffle(data)
    label = min(data) / max(data)
...
MLAE = np.mean(np.log2(np.abs(y_pred - y_test) * 100 + .125))
